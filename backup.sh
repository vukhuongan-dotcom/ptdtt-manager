...
if ! git show-ref --verify --quiet refs/heads/backup-data; then
    git checkout --orphan backup-data --quiet
    git rm -rf . --quiet 2>/dev/null || true
else
    git checkout backup-data --quiet
fi
...
if command -v rclone &>/dev/null && rclone listremotes 2>/dev/null | grep -q "gdrive:"; then
    ...
    rclone copy "$BACKUP_DIR/db_${DATE}.json" "$GDRIVE_BACKUP/" --quiet 2>/dev/null
    rclone copy "$BACKUP_DIR/auth_${DATE}.json" "$GDRIVE_BACKUP/" --quiet 2>/dev/null
    if [ -d "$BACKUP_DIR/shcm-files" ]; then
        rclone sync "$BACKUP_DIR/shcm-files/" "$GDRIVE_BACKUP/shcm-files/" --quiet 2>/dev/null
    fi
