// ===== REAL STAFF DATA FROM DS NHÂN VIÊN KHOA ĐTT.xlsx =====
const SAMPLE_STAFF = [
    {
        "color": "#8b5cf6",
        "cơHữu": true,
        "email": "bsphuhuu2012@gmail.com",
        "id": 1,
        "name": "Nguyễn Phú Hữu",
        "phone": "0918650345",
        "role": "BS Trưởng khoa",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "TS. BSCKII",
        "dob": "1980-11-17",
        "gender": "Nam"
    },
    {
        "color": "#06b6d4",
        "cơHữu": true,
        "email": "vukhuongan@gmail.com",
        "id": 2,
        "name": "Vũ Khương An",
        "phone": "0909927154",
        "role": "BS Phó trưởng khoa",
        "status": "active",
        "statusFrom": "2026-07-22",
        "statusNote": "",
        "statusTo": "2026-07-24",
        "statusType": "business",
        "title": "BSCKII",
        "dob": "1992-03-19",
        "gender": "Nam"
    },
    {
        "color": "#ec4899",
        "cơHữu": true,
        "email": "teresathuynguyen82@gmail.com",
        "id": 3,
        "name": "Nguyễn Thị Ngọc Thùy",
        "phone": "0908261563",
        "role": "Điều dưỡng trưởng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "1982-02-02",
        "gender": "Nữ"
    },
    {
        "color": "#f59e0b",
        "cơHữu": false,
        "email": "drdomtuan29@gmail.com",
        "id": 4,
        "name": "Vũ Ngọc Anh Tuấn",
        "note": "Giảng viên PNT",
        "phone": "0902770599",
        "role": "Bác sĩ chính",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "BSCKII",
        "dob": "1983-09-02",
        "gender": "Nam"
    },
    {
        "color": "#3b82f6",
        "cơHữu": false,
        "email": "drhaubui@gmail.com",
        "id": 5,
        "name": "Bùi Hồng Minh Hậu",
        "note": "BS phòng KHTH",
        "phone": "0933024160",
        "role": "Bác sĩ chính",
        "status": "active",
        "title": "BSCKII",
        "dob": "1988-10-17",
        "gender": "Nam"
    },
    {
        "color": "#14b8a6",
        "cơHữu": false,
        "email": "chinguyenvo@outlook.com",
        "id": 6,
        "name": "Võ Chí Nguyện",
        "note": "Giảng viên Tân Tạo",
        "phone": "0939530499",
        "role": "Bác sĩ chính",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "BSCKII",
        "dob": "1989-02-02",
        "gender": "Nam"
    },
    {
        "color": "#f97316",
        "cơHữu": true,
        "email": "dr.phamvinhphu@gmail.com",
        "id": 7,
        "name": "Phạm Vĩnh Phú",
        "phone": "0937462877",
        "role": "Bác sĩ chính",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "BSCKI",
        "dob": "1995-03-27",
        "gender": "Nam"
    },
    {
        "color": "#a855f7",
        "cơHữu": true,
        "email": "drgiaoquy@gmail.com",
        "id": 8,
        "name": "Giao Hữu Trường Quy",
        "phone": "0975739589",
        "role": "Bác sĩ chính",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "BSCKI",
        "dob": "1995-12-11",
        "gender": "Nam"
    },
    {
        "color": "#10b981",
        "cơHữu": true,
        "email": "drminhducth@gmail.com",
        "id": 9,
        "name": "Trịnh Hoàng Minh Đức",
        "phone": "0906072054",
        "role": "Bác sĩ chính",
        "status": "active",
        "title": "BSCKI",
        "dob": "1996-10-08",
        "gender": "Nam"
    },
    {
        "color": "#06b6d4",
        "cơHữu": true,
        "email": "trannhuduc97@gmail.com",
        "id": 10,
        "name": "Trần Như Đức",
        "phone": "0349982469",
        "role": "Bác sĩ chính",
        "status": "active",
        "statusFrom": "2026-07-27",
        "statusNote": "",
        "statusTo": "2026-07-29",
        "statusType": "dayoff",
        "title": "BSCKI",
        "dob": "1997-01-11",
        "gender": "Nam"
    },
    {
        "color": "#8b5cf6",
        "cơHữu": true,
        "email": "chicothelalehoan@gmail.com",
        "id": 11,
        "name": "Lê Văn Hoan",
        "phone": "0344254024",
        "role": "Bác sĩ chính",
        "status": "active",
        "title": "BSCKI",
        "dob": "1997-07-27",
        "gender": "Nam"
    },
    {
        "color": "#ec4899",
        "cơHữu": true,
        "email": "tuyetminh061297@gmail.com",
        "id": 12,
        "name": "Phạm Thị Tuyết Minh",
        "phone": "0775696917",
        "role": "Bác sĩ chính",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "BSCKI",
        "dob": "1997-12-06",
        "gender": "Nữ"
    },
    {
        "color": "#f97316",
        "cơHữu": false,
        "email": "nmnp252@gmail.com",
        "id": 16,
        "name": "Nguyễn Minh Nguyên Phương",
        "phone": "0977347794",
        "role": "Bác sĩ học viên",
        "status": "active",
        "title": "BS. Học viên",
        "dob": "1997-12-25",
        "gender": "Nam"
    },
    {
        "color": "#14b8a6",
        "cơHữu": true,
        "email": "phamthaot84@gmail.com",
        "id": 24,
        "name": "Phạm Thị Thanh Thảo",
        "phone": "0938231578",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "1984-06-30",
        "gender": "Nữ"
    },
    {
        "color": "#f97316",
        "cơHữu": true,
        "email": "tranphuongquan45@gmail.com",
        "id": 25,
        "name": "Trần Phương Quan",
        "phone": "0328288328",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "ĐDCKI",
        "dob": "1998-01-09",
        "gender": "Nam"
    },
    {
        "color": "#a855f7",
        "cơHữu": true,
        "email": "hien.nguyen.nhu84@gmail.com",
        "id": 26,
        "name": "Nguyễn Như Hiền",
        "phone": "0765825528",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "2026-07-15",
        "statusNote": "",
        "statusTo": "2026-07-15",
        "statusType": "dayoff",
        "title": "CNĐD",
        "dob": "1984-02-06",
        "gender": "Nữ"
    },
    {
        "color": "#10b981",
        "cơHữu": true,
        "email": "Phant151096@gmail.com",
        "id": 27,
        "name": "Phan Thị Cẩm Tiên",
        "phone": "0903640318",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "2026-08-10",
        "statusNote": "",
        "statusTo": "2026-08-10",
        "statusType": "dayoff",
        "title": "CNĐD",
        "dob": "1996-10-15",
        "gender": "Nữ"
    },
    {
        "color": "#06b6d4",
        "cơHữu": true,
        "email": "tranthanhdanh9191@gmail.com",
        "id": 28,
        "name": "Trần Thanh Danh",
        "phone": "0903390991",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "ĐD",
        "dob": "1991-09-20",
        "gender": "Nữ"
    },
    {
        "color": "#8b5cf6",
        "cơHữu": true,
        "email": "phanthithuytien1606@gmail.com",
        "id": 29,
        "name": "Phan Thị Thủy Tiên",
        "phone": "0392400393",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "1994-08-14",
        "gender": "Nữ"
    },
    {
        "color": "#ec4899",
        "cơHữu": true,
        "email": "huyennguyen195177@gmal.com",
        "id": 30,
        "name": "Nguyễn Thị Huyền",
        "phone": "0327091879",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "ĐDCKI",
        "dob": "1996-04-21",
        "gender": "Nữ"
    },
    {
        "color": "#f59e0b",
        "cơHữu": true,
        "email": "huynhkimxuanhang@gmail.com",
        "id": 31,
        "name": "Huỳnh Kim Xuân Hằng",
        "phone": "0773136412",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "1999-05-05",
        "gender": "Nữ"
    },
    {
        "color": "#3b82f6",
        "cơHữu": true,
        "email": "trinhbtm123@gmail.com",
        "id": 32,
        "name": "Bùi Thị Mộng Trinh",
        "phone": "0392371516",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "1993-10-05",
        "gender": "Nữ"
    },
    {
        "color": "#14b8a6",
        "cơHữu": true,
        "email": "thutranglee1102@gmail.com",
        "id": 33,
        "name": "Lê Thị Thu Trang",
        "phone": "0987239059",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "1989-02-11",
        "gender": "Nữ"
    },
    {
        "color": "#f97316",
        "cơHữu": true,
        "email": "nhthao2008@gmail.com",
        "id": 34,
        "name": "Lê Thị Như Thảo",
        "phone": "0386733697",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CĐĐD"
    },
    {
        "color": "#a855f7",
        "cơHữu": true,
        "email": "nguyenhoangdieutram.2b@gmail.com",
        "id": 35,
        "name": "Nguyễn Hoàng Diệu Trâm",
        "phone": "0703613583",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "2002-03-19",
        "gender": "Nữ"
    },
    {
        "color": "#10b981",
        "cơHữu": true,
        "email": "lyhoangduy522002@gmail.com",
        "id": 36,
        "name": "Lý Hoàng Duy",
        "phone": "0796647516",
        "role": "Điều dưỡng",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "2002-02-05",
        "gender": "Nam"
    },
    {
        "color": "#06b6d4",
        "cơHữu": true,
        "email": "buithitran070291@gmail.com",
        "id": 37,
        "name": "Bùi Thị Trưng",
        "phone": "0982553527",
        "role": "Hộ lý",
        "status": "active",
        "title": "HL",
        "dob": "1991-02-07",
        "gender": "Nữ"
    },
    {
        "color": "#8b5cf6",
        "cơHữu": true,
        "email": "nguenanh2701@gmail.com",
        "id": 38,
        "name": "Lê Thị Thúy An",
        "phone": "0907052388",
        "role": "Hộ lý",
        "status": "active",
        "statusFrom": "",
        "statusNote": "",
        "statusTo": "",
        "statusType": "active",
        "title": "HL",
        "dob": "1986-04-08",
        "gender": "Nữ"
    },
    {
        "color": "#ec4899",
        "cơHữu": true,
        "email": "hieuhuynh311281@gmail.com",
        "id": 39,
        "name": "Huỳnh Văn Hiếu",
        "phone": "0907473372",
        "role": "Hộ lý",
        "status": "active",
        "title": "HL",
        "dob": "1981-12-31",
        "gender": "Nam"
    },
    {
        "color": "#f59e0b",
        "cơHữu": true,
        "email": "nguyenthihoa3121983@gmail.com",
        "id": 40,
        "name": "Nguyễn Thị Hoa",
        "phone": "0909324233",
        "role": "Thư ký",
        "status": "active",
        "title": "TK",
        "dob": "1983-12-03",
        "gender": "Nữ"
    },
    {
        "color": "#3b82f6",
        "email": "khoibtsk1@gmail.com",
        "id": 46,
        "name": "Nguyễn Ngọc Minh Khôi",
        "phone": "0848635567",
        "role": "Bác sĩ học viên",
        "startDate": "2026-08-03",
        "status": "active",
        "statusType": "active",
        "title": "BSNT",
        "dob": "1999-02-11",
        "gender": "Nam"
    },
    {
        "color": "#a855f7",
        "email": "",
        "id": 43,
        "name": "Nguyễn Đức Thiên Phú",
        "phone": "0917365115",
        "role": "Bác sĩ học viên",
        "startDate": "2026-08-03",
        "status": "active",
        "statusType": "active",
        "title": "BSNT",
        "statusFrom": "",
        "statusTo": "",
        "statusNote": "",
        "dob": "2000-07-02",
        "gender": "Nam"
    },
    {
        "color": "#f59e0b",
        "email": "",
        "id": 44,
        "name": "Phùng Bùi Tuấn Kiệt",
        "phone": "0903006976",
        "role": "Bác sĩ học viên",
        "startDate": "2026-08-03",
        "status": "active",
        "statusType": "active",
        "title": "BSNT",
        "dob": "2000-05-06",
        "gender": "Nam"
    },
    {
        "color": "#f59e0b",
        "cơHữu": true,
        "email": "nguyenducluana4@gmail.com",
        "id": 45,
        "name": "Nguyễn Đức Luân",
        "phone": "0346814876",
        "role": "Bác sĩ học viên",
        "startDate": "2026-08-03",
        "status": "active",
        "statusType": "active",
        "title": "BSNT",
        "dob": "2000-10-15",
        "gender": "Nam"
    },
    {
        "color": "#10b981",
        "email": "oanhu642003@gmail.com",
        "id": 49,
        "name": "Ừng Phương Minh Oanh",
        "phone": "0931042403",
        "role": "Điều dưỡng",
        "startDate": "2026-07-31",
        "status": "active",
        "statusType": "active",
        "title": "CNĐD",
        "dob": "2003-04-06",
        "gender": "Nữ"
    },
    {
        "color": "#10b981",
        "cơHữu": false,
        "dob": "2001-03-25",
        "email": "bshbsang@gmail.com",
        "id": 47,
        "mshv": "NTNG2503",
        "name": "Hoàng Bá Sang",
        "phone": "0335846050",
        "role": "Bác sĩ học viên",
        "startDate": "2026-08-03",
        "status": "active",
        "statusType": "active",
        "title": "BSNT",
        "gender": "Nam"
    },
    {
        "color": "#f59e0b",
        "cơHữu": false,
        "dob": "2001-04-16",
        "email": "",
        "id": 48,
        "mshv": "NTNG2505",
        "name": "Lê Minh Thành",
        "phone": "0965663267",
        "role": "Bác sĩ học viên",
        "startDate": "2026-08-03",
        "status": "active",
        "statusType": "active",
        "title": "BSNT",
        "gender": "Nam"
    },
    {
        "color": "#f97316",
        "cơHữu": false,
        "email": "tudang2806@gmail.com",
        "id": 50,
        "name": "Đặng Cẩm Tú",
        "phone": "0836629554",
        "role": "Bác sĩ học viên",
        "startDate": "2026-07-31",
        "status": "active",
        "title": "BS. Học viên",
        "dob": "1998-06-28",
        "gender": "Nữ"
    },
    {
        "color": "#3b82f6",
        "email": "bichchaudo1989@gmail.com",
        "id": 51,
        "name": "Đỗ Thị Bích Châu",
        "phone": "0978888240",
        "role": "Điều dưỡng",
        "status": "active",
        "statusType": "active",
        "title": "CNĐD"
    }
];

// ===== EXTERNAL DOCTORS (BS ngoài khoa hỗ trợ) =====
const SAMPLE_EXTERNAL_DOCTORS = [
    { id: 101, name: 'Lương Thanh Tùng', title: 'BSCKII', position: 'Phó Giám đốc', department: 'Ban Giám đốc', note: 'Phó giám đốc', color: '#6366f1' },
    { id: 102, name: 'Hoàng Vĩnh Chúc', title: 'BSCKII', position: 'BS Tham vấn', department: 'Khoa PTĐTT', note: '', color: '#0ea5e9' },
    { id: 103, name: 'Trần Thiện Hoà', title: 'BS', position: 'Bác sĩ', department: 'Ngoại Tiêu hoá', note: 'Ngoại Tiêu hoá', color: '#22c55e' },
];

const SAMPLE_TASKS = [
    { id: 1, title: 'Chuẩn bị hội chẩn ca BN Nguyễn Văn A', desc: 'Ung thư trực tràng T3N1M0', assignee: 1, priority: 'high', deadline: '2026-03-24', status: 'todo' },
    { id: 2, title: 'Hoàn thành báo cáo ca mổ robot tháng 3', desc: 'Tổng hợp số liệu 15 ca mổ robot', assignee: 2, priority: 'medium', deadline: '2026-03-28', status: 'todo' },
    { id: 3, title: 'Cập nhật protocol chăm sóc sau mổ', desc: 'Chỉnh sửa theo guideline ACS 2026', assignee: 7, priority: 'medium', deadline: '2026-03-30', status: 'doing' },
    { id: 4, title: 'Phẫu thuật nội soi cắt đại tràng phải', desc: 'BN Trần Thị B, giường 12', assignee: 9, priority: 'high', deadline: '2026-03-23', status: 'doing' },
    { id: 5, title: 'Viết đề cương NCKH robot phẫu thuật', desc: 'Đề tài so sánh robot vs nội soi', assignee: 2, priority: 'low', deadline: '2026-04-15', status: 'todo' },
    { id: 6, title: 'Đào tạo ĐDV kỹ thuật chăm sóc stoma', desc: 'Buổi training cho 5 điều dưỡng mới', assignee: 3, priority: 'medium', deadline: '2026-03-25', status: 'done' },
    { id: 7, title: 'Kiểm kê vật tư tiêu hao tháng 3', desc: 'Stapler, trocar, mesh', assignee: 40, priority: 'low', deadline: '2026-03-31', status: 'done' },
    { id: 8, title: 'Hội chẩn đa chuyên khoa - BN ung thư', desc: 'Ca phức tạp cần hội chẩn UHBM, Nội soi, CĐHA', assignee: 1, priority: 'high', deadline: '2026-03-24', status: 'doing' },
];

const SAMPLE_PLANS = [
    { id: 1, title: 'Họp giao ban khoa', date: '2026-03-24', time: '07:30', location: 'Phòng họp khoa', type: 'meeting', responsible: 1, note: '' },
    { id: 2, title: 'Hội chẩn GS Ninh', date: '2026-03-25', time: '10:00', location: 'Phòng hội chẩn', type: 'consultation', responsible: 2, note: '3 ca phức tạp' },
    { id: 3, title: 'Đào tạo kỹ thuật khâu nối', date: '2026-03-26', time: '14:00', location: 'Phòng mổ Lab', type: 'training', responsible: 7, note: 'Cho BSNT năm 2' },
    { id: 4, title: 'Hội nghị VASEL 2026', date: '2026-03-28', time: '08:00', location: 'Khách sạn Rex', type: 'conference', responsible: 2, note: 'Báo cáo robot' },
    { id: 5, title: 'Sinh hoạt chuyên môn', date: '2026-03-27', time: '13:30', location: 'Phòng họp khoa', type: 'meeting', responsible: 1, note: 'Review ca biến chứng' },
    { id: 6, title: 'Khám đoàn Côn Đảo', date: '2026-04-01', time: '06:00', location: 'Côn Đảo', type: 'other', responsible: 2, note: 'Đợt 2 năm 2026' },
    { id: 7, title: 'Họp khoa hàng tuần', date: '2026-03-31', time: '07:30', location: 'Phòng họp khoa', type: 'meeting', responsible: 1, note: '' },
    { id: 8, title: 'Tập huấn ACS', date: '2026-04-03', time: '09:00', location: 'Hội trường BV', type: 'training', responsible: 1, note: 'Module Patient Safety' },
];

const SAMPLE_PATIENTS = [
    { id: 1, name: 'Nguyễn Văn A', age: 58, gender: 'Nam', bed: 'G1-01', diagnosis: 'K trực tràng 1/3 giữa', admitDate: '2026-03-18', status: 'pre-op', doctor: 2 },
    { id: 2, name: 'Trần Thị B', age: 65, gender: 'Nữ', bed: 'G1-03', diagnosis: 'K đại tràng phải', admitDate: '2026-03-20', status: 'post-op', doctor: 9 },
    { id: 3, name: 'Lê Văn C', age: 72, gender: 'Nam', bed: 'G1-05', diagnosis: 'K đại tràng sigma', admitDate: '2026-03-15', status: 'active', doctor: 1 },
    { id: 4, name: 'Phạm Thị D', age: 45, gender: 'Nữ', bed: 'G2-02', diagnosis: 'Polyp đại tràng', admitDate: '2026-03-22', status: 'pre-op', doctor: 7 },
    { id: 5, name: 'Hoàng Văn E', age: 55, gender: 'Nam', bed: 'G2-04', diagnosis: 'Trĩ nội độ IV', admitDate: '2026-03-21', status: 'post-op', doctor: 8 },
    { id: 6, name: 'Đỗ Thị F', age: 63, gender: 'Nữ', bed: 'G1-07', diagnosis: 'K trực tràng 1/3 dưới', admitDate: '2026-03-17', status: 'active', doctor: 2 },
    { id: 7, name: 'Vương Văn G', age: 48, gender: 'Nam', bed: 'G2-06', diagnosis: 'Rò hậu môn phức tạp', admitDate: '2026-03-22', status: 'pre-op', doctor: 10 },
    { id: 8, name: 'Mai Thị H', age: 70, gender: 'Nữ', bed: 'G1-09', diagnosis: 'Tắc ruột do dính', admitDate: '2026-03-19', status: 'active', doctor: 1 },
];

// ===== SAMPLE SCHEDULE: 23.03.2026 – 29.03.2026 =====
const SAMPLE_SCHEDULES = [{
    id: 1,
    weekKey: '2026-03-23',
    startDate: '2026-03-23',
    endDate: '2026-03-29',
    notes: 'BS Hữu nghỉ phép thứ 6',
    positions: {
        trucKhoa: {
            // T2: Tuấn, N.Đức, Định, Hậu NT
            T2_0:4, T2_1:10, T2_2:22, T2_3:23,
            // T3: M.Đức, Hoan, Hoàng
            T3_0:9, T3_1:11, T3_2:14,
            // T4: Nguyện, Minh, Trâm Anh
            T4_0:6, T4_1:12, T4_2:15,
            // T5: Quy, N.Đức, Huy
            T5_0:8, T5_1:10, T5_2:13,
            // T6: Phú, Hoan, Ý
            T6_0:7, T6_1:11, T6_2:17,
        },
        sieuAm: {
            T2_0:4, T3_0:9, T4_0:6, T5_0:7, T6_0:8,
        },
        pkB023: {
            T2_0:5, T3_0:1, T4_0:4, T5_0:2, T6_0:6,
        },
        pkB020: {
            // T2: Sáng + Chiều Vĩnh Phú | T3: Sáng An
            T2_0:7, T2_1:7,
            T3_0:2,
        },
        pkK001: {
            // T2: Sáng An, Chiều M.Đức | T6: Sáng + Chiều Quy
            T2_0:2, T2_1:9,
            T6_0:8, T6_1:8,
        },
        mo: {
            // T2: Hữu, Nguyện, M.Đức, Hoan, Trâm Anh, Hoàng, Huy, Ý
            T2_0:1, T2_1:6, T2_2:9, T2_3:11, T2_4:15, T2_5:14, T2_6:13, T2_7:17,
            // T3: Tuấn, Phú, Quy, N.Đức, Huy, Định, Ý
            T3_0:4, T3_1:7, T3_2:8, T3_3:10, T3_4:13, T3_5:22, T3_6:17,
            // T4: An, Phú, M.Đức, Hoan, Huy, Ý, Hậu NT
            T4_0:2, T4_1:7, T4_2:9, T4_3:11, T4_4:13, T4_5:17, T4_6:23,
            // T5: Hữu, Tuấn, Quy, Minh, Trâm Anh, Hoàng, Định, Hậu NT
            T5_0:1, T5_1:4, T5_2:8, T5_3:12, T5_4:15, T5_5:14, T5_6:22, T5_7:23,
            // T6: Hậu, Nguyện, Minh, Trâm Anh, Hoàng, Định, Hậu NT
            T6_0:5, T6_1:6, T6_2:12, T6_3:15, T6_4:14, T6_5:22, T6_6:23,
            // T7: M.Đức, Hoàng, Định
            T7_0:9, T7_1:14, T7_2:22,
        },
        trucBCN: {
            T2_0:2, T3_0:1, T4_0:1, T5_0:2, T6_0:2, T7_0:1, CN_0:2,
        },
        trucBV: {
            // T2: Phú, Hoàng
            T2_0:7, T2_1:14,
            // T3: M.Đức
            T3_0:9,
            // T4: Định
            T4_0:22,
            // T5: An, N.Đức
            T5_0:2, T5_1:10,
            // T6: Hoan, Hậu NT
            T6_0:11, T6_1:23,
            // T7: Minh, Trâm Anh
            T7_0:12, T7_1:15,
            // CN: Quy, Ý
            CN_0:8, CN_1:17,
        },
        trucDD: {
            // T2: Hiền, Trinh
            T2_0:26, T2_1:32,
            // T3: Huyền, Trang
            T3_0:30, T3_1:33,
            // T4: Tiên, C.Tiên
            T4_0:29, T4_1:27,
            // T5: Danh, Hằng
            T5_0:28, T5_1:31,
            // T6: Hiền, Trinh
            T6_0:26, T6_1:32,
            // T7: Huyền, Trang
            T7_0:30, T7_1:33,
            // CN: Tiên, C.Tiên
            CN_0:29, CN_1:27,
        },
    }
}];

const MONTHLY_SURGERIES = [22, 18, 25, 20, 28, 24, 30, 26, 32, 28, 24, 27];
const MONTH_LABELS = ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12'];

// ===== SAMPLE SHCM (Sinh hoạt Chuyên môn) =====
const SAMPLE_SHCM = [
    { id:1, doctorName:'Bs. Vũ Ngọc Anh Tuấn', doctorId:4, title:'Phân giai đoạn ung thư đường tiêu hóa và cách làm hồ sơ xuất viện', status:'done', presentDate:'2025-04-15' },
    { id:2, doctorName:'BS. Võ Chí Nguyện', doctorId:6, title:'Chuẩn bị bệnh nhân trước phẫu thuật ung thư đại trực tràng', status:'done', presentDate:'2025-04-29' },
    { id:3, doctorName:'BS. Giao Hữu Trường Quy', doctorId:8, title:'Chăm sóc bệnh nhân sau phẫu thuật ung thư đại trực tràng', status:'done', presentDate:'2025-06-17' },
    { id:4, doctorName:'BS. Vũ Khương An', doctorId:2, title:'Cập nhật trong chẩn đoán và điều trị ung thư đại trực tràng 2025', status:'done', presentDate:'2025-05-07' },
    { id:5, doctorName:'BS. Vũ Khương An', doctorId:2, title:'Hội chẩn tham vấn (Thầy Chúc) trường hợp rò tiêu hóa. Kinh nghiệm xử trí u đại tràng ngang.', status:'done', presentDate:'2025-07-24' },
    { id:6, doctorName:'BS. Vũ Ngọc Anh Tuấn', doctorId:4, title:'Ứng dụng laser trong phẫu thuật trĩ, rò', status:'done', presentDate:'2025-11-06' },
    { id:7, doctorName:'BS. Phạm Vĩnh Phú', doctorId:7, title:'Một số kinh nghiệm trong công bố và báo cáo quốc tế', status:'done', presentDate:'2025-12-22' },
    { id:8, doctorName:'BS. Vũ Khương An', doctorId:2, title:'Ý tưởng báo cáo khoa học trong năm 2026, các lỗi thường gặp khi thực hiện BAĐT', status:'done', presentDate:'2026-01-26' },
    { id:9, doctorName:'BS. Vũ Khương An', doctorId:2, title:'Tổng kết triển khai mô hình POD bệnh phòng & Định hướng luân chuyển Nội trú kỳ mới', status:'done', presentDate:'2026-02-23' },
    { id:10, doctorName:'BS. Trịnh Hoàng Minh Đức', doctorId:9, title:'Cập nhật điều trị polyp đại trực tràng', status:'done', presentDate:'2026-03-09' },
    { id:11, doctorName:'BS. Phạm Vĩnh Phú', doctorId:7, title:'Cập nhật hướng dẫn chẩn đoán và điều trị Helicobacter pylori', status:'done', presentDate:'2026-03-23' },
    { id:12, doctorName:'BS. Võ Chí Nguyện', doctorId:6, title:'Có nên hạ góc lách thường quy trong phẫu thuật cắt trực tràng?', status:'done', presentDate:'2026-04-06' },
    { id:13, doctorName:'BS. Võ Chí Nguyện', doctorId:6, title:'Hồi tràng ra da/ Phẫu thuật cắt trực tràng: Kỹ thuật và biến chứng liên quan', status:'done', presentDate:'2026-04-20' },
    { id:14, doctorName:'BS. Vũ Ngọc Anh Tuấn', doctorId:4, title:'Ứng dụng laser trong điều trị bệnh lý condyloma', status:'done', presentDate:'2026-05-04' },
    { id:15, doctorName:'TS. BS Nguyễn Phú Hữu - BS. Vũ Khương An', doctorId:1, title:'Tổng duyệt chuẩn bị cho “Hội nghị khoa học thường niên phẫu thuật Đại trực tràng Việt Nam năm 2026”', status:'done', presentDate:'2026-05-25' },
    { id:16, doctorName:'Khoa PTĐTT', doctorId:2, title:'Tổng duyệt chuẩn bị cho Hội nghị khoa học thường niên phẫu thuật Đại trực tràng Việt Nam năm 2026 (Đợt 2)', status:'done', presentDate:'2026-06-01' },
    { id:17, doctorName:'Bs. Vũ Khương An', doctorId:2, title:'Can thiệp ngoại khoa trong xuất huyết tiêu hoá từ Ruột non (SHCM BV)', status:'done', presentDate:'2026-06-25' },
    { id:18, doctorName:'BS. Võ Chí Nguyện - BS. Vũ Khương An - BSNT. Trâm Anh', doctorId:6, title:'Chuẩn bị bệnh nhân mổ chương trình: các phẫu thuật lớn', status:'done', presentDate:'2026-07-06' },
    { id:19, doctorName:'Khoa PTĐTT', doctorId:2, title:'Sinh hoạt CLB “Hậu môn nhân tạo” Bệnh viện Bình Dân 2026', status:'done', presentDate:'2026-07-18' },
    { id:20, doctorName:'BS. Phạm Vĩnh Phú', doctorId:7, title:'Thao tác cơ bản tại phòng mổ', status:'done', presentDate:'2026-08-24' },
    { id:21, doctorName:'BS. Lê Văn Hoan', doctorId:11, title:'Quy trình CODE BLUE', status:'done', presentDate:'2026-09-07' },
    { id:22, doctorName:'BS. Trần Như Đức', doctorId:10, title:'Xử trí u dưới niêm dạ dày, tá tràng với các kích thước khác nhau', status:'pending', presentDate:'2026-09-21' },
    { id:23, doctorName:'BS. Giao Hữu Trường Quy', doctorId:8, title:'Phẫu thuật nội soi điều trị thoát vị bẹn - TAPP vs TEP', status:'pending', presentDate:'2026-10-05' },
    { id:24, doctorName:'BS. Lê Văn Hoan', doctorId:11, title:'Cập nhật hướng dẫn sử dụng kháng sinh dự phòng, kháng sinh điều trị', status:'pending', presentDate:'2026-10-19' },
    { id:25, doctorName:'BS. Phạm Thị Tuyết Minh', doctorId:12, title:'Ứng dụng giảm đau đa mô thức trong hậu phẫu', status:'pending', presentDate:'2026-11-02' },
    { id:26, doctorName:'BS. Lê Văn Hoan', doctorId:11, title:'Cập nhật hướng dẫn chẩn đoán và điều trị viêm túi thừa đại tràng', status:'pending', presentDate:'2026-11-16' },
    { id:27, doctorName:'BS. Võ Chí Nguyện', doctorId:6, title:'Cập nhật chẩn đoán điều trị IBS', status:'pending', presentDate:'2026-11-30' },
    { id:28, doctorName:'BS. Lê Văn Hoan', doctorId:11, title:'Xử trí tắc ruột do u đại trực tràng: PTNS mở HMNT trên dòng?', status:'pending', presentDate:'2026-12-14' },
    { id:29, doctorName:'BS. Trịnh Hoàng Minh Đức', doctorId:9, title:'Chẩn đoán và xử trí tắc mạch máu mạc treo ruột', status:'pending', presentDate:'2026-12-28' },
    { id:30, doctorName:'BS. Lê Văn Hoan', doctorId:11, title:'Ung thư đại trực tràng đồng thời: Tiếp cận và xử trí', status:'pending', presentDate:'2027-01-11' },
    { id:31, doctorName:'BS. Trần Như Đức', doctorId:10, title:'DNA tự do của khối u trong máu (ctDNA) và quản lý ung thư đại trực tràng.', status:'pending', presentDate:'2027-01-25' },
    { id:32, doctorName:'BS. Phạm Thị Tuyết Minh', doctorId:12, title:'Hiệu quả của tư vấn di truyền trong điều trị và dự phòng ung thư đại trực tràng có tính chất gia đình.', status:'pending', presentDate:'2027-02-15' },
    { id:33, doctorName:'BS. Giao Hữu Trường Quy', doctorId:8, title:'Động mạch đại tràng trái: Các biến thể giải phẫu và cách xác định chúng', status:'pending', presentDate:'2027-03-01' },
    { id:34, doctorName:'BS. Giao Hữu Trường Quy', doctorId:8, title:'Các cách tiếp cận trong phẫu thuật cắt đại tràng phải', status:'pending', presentDate:'2027-03-15' },
    { id:35, doctorName:'Bs. Vũ Khương An', doctorId:2, title:'Sinh lý bệnh của sự lành miệng nối tiêu hoá và ứng dụng thực tiễn', status:'pending', presentDate:'2027-03-29' }
];
