...
const StaffPage = {
    ...
    getFiltered() {
        ...
        if (this.searchQuery) {
            ...
            staff = staff.filter(s => s.name.toLowerCase().includes(q) || s.role.toLowerCase().includes(q));
        }
        return staff;
    },

    setFilter(f) { this.currentFilter = f; App.renderCurrentPage(); },
    search(q) { this.searchQuery = q; App.renderCurrentPage(); document.getElementById('staff-search')?.focus(); },
